...
if(D.funnel){
  ...
  const clRows=moR.map((r,i)=>{ const a0=r.apd0!=null?r.apd0:0; const clean=r.ftds-r.pba-a0; const p=moTot[i+1].p; return {cells:[r.m+(r.apd0==null?' †':''), num(r.ftds), num(r.pba), r.apd0!=null?num(r.apd0):'—', num(clean), pct1(clean/r.ftds), gbpK(p), gbp(div(p,r.ftds)), `<b>${gbp(div(p,clean))}</b>`]}; });
  const clTot=moR.reduce((a,r,i)=>{ const a0=r.apd0!=null?r.apd0:0; a.f+=r.ftds; a.pba+=r.pba; a.a0+=a0; a.clean+=r.ftds-r.pba-a0; a.p+=moTot[i+1].p; return a; },{f:0,pba:0,a0:0,clean:0,p:0});
  clRows.push({cls:'tot',cells:['YTD', num(clTot.f), num(clTot.pba), num(clTot.a0)+' (Jan–May)', num(clTot.clean), pct1(clTot.clean/clTot.f), gbpM(clTot.p), gbp(div(clTot.p,clTot.f)), `<b>${gbp(div(clTot.p,clTot.clean))}</b>`]});
...
<div style="margin-top:14px">${tbl([{t:'Month'},{t:'FTDs',r:1},{t:'Current PBA',r:1},{t:'APD0',r:1},{t:'Clean FTDs',r:1},{t:'Clean %',r:1},{t:'Net PLTV',r:1},{t:'PLTV/FTD (all)',r:1},{t:'True PLTV/FTD (clean)',r:1}], clRows)}</div>
<p class="note">Clean FTDs = FTDs − Current PBA (official ThoughtSpot series) − APD0 (zero paid playing days). The two populations overlap to an unknown degree (player-level PBA isn't accessible yet — DD-596), so Clean FTDs is a conservative floor and true PLTV/FTD a ceiling. † Jun &amp; Jul remove PBA only — APD0 unavailable (gameplay mart loaded to 22 Jun). Net PLTV keeps the whole cohort's value (PBA/APD0 players contribute ≈£0), so true PLTV/FTD reads as net value per genuine customer.</p>
