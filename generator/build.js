...
if(D.funnel){
  ...
  const moRows=moR.map(r=>({cells:[r.m, num(r.ftds), num(r.pba), `<span class="pill ${r.pbaP>=2?'red':r.pbaP>=1?'amber':'green'}">${r.pbaP.toFixed(2)}%</span>`, num(r.fr), `<span class="pill ${r.frP>=2*frB?'red':r.frP>=1.25*frB?'amber':'green'}">${r.frP.toFixed(1)}%</span>`, r.dupP.toFixed(1)+'%', num(r.apd2), pct1(r.apdP/100), num(r.pp8), pct1(r.ppP/100), r.qore!=null?num(r.qore):'—', r.qP!=null?pct1(r.qP/100):'—']}));
  moRows.push({cls:'tot',cells:['YTD', num(fyt.ftds), num(fyt.pba), pct1(pbaB/100), num(fyt.fr), pct1(frB/100), pct1(dupB/100), num(fyt.apd2), pct1(apdB/100), num(fyt.pp8), pct1(ppB/100), num(qTot)+' (Jan–Jun)', pct1(qB/100)]});
  const wkRows=wkR.map((r,i)=>({cells:[r.w+(i===wkR.length-1?' (WTD)':''), num(r.ftds), r.pbaP.toFixed(2)+'%', r.frP.toFixed(1)+'%', r.dupP.toFixed(1)+'%', num(r.apd2), pct1(r.apdP/100), num(r.pp8), pct1(r.ppP/100)]}));
  const chRows=chR.map(r=>({cells:[r.ch, num(r.f), `<span class="pill ${r.pbaP>Math.max(1.5,3*pbaB)?'red':r.pbaP>2*pbaB?'amber':'green'}">${r.pbaP.toFixed(2)}%</span>`, `<span class="pill ${r.frP>2*frB?'red':r.frP>1.25*frB?'amber':'green'}">${r.frP.toFixed(1)}%</span>`, `<span class="pill ${r.dupP>dupB+5?'red':r.dupP>dupB+2?'amber':'green'}">${r.dupP.toFixed(1)}%</span>`, pct1(r.apdP/100), `<span class="pill ${r.ppP>=ppB?'green':r.ppP>=ppB*0.75?'amber':'red'}">${r.ppP.toFixed(1)}%</span>`, r.avgpp.toFixed(2), div(r.pn,r.fm)?gbp(div(r.pn,r.fm)):'—']}));
...
${chartbox('c_fun_mo')}
<div style="margin-top:14px">${tbl([{t:'Month'},{t:'FTDs',r:1},{t:'PBA',r:1},{t:'PBA %',r:1},{t:'Fraud',r:1},{t:'Fraud %',r:1},{t:'Dup %',r:1},{t:'APD2+',r:1},{t:'APD2+ %',r:1},{t:'PP 8–10',r:1},{t:'PP %',r:1},{t:'Qore FTDs',r:1},{t:'Qore %',r:1}], moRows)}</div>
...
${chartbox('c_fun_wk')}
<div style="margin-top:14px">${tbl([{t:'Week (w/c)'},{t:'FTDs',r:1},{t:'PBA %',r:1},{t:'Fraud %',r:1},{t:'Dup %',r:1},{t:'APD2+',r:1},{t:'APD2+ %',r:1},{t:'PP 8–10',r:1},{t:'PP %',r:1}], wkRows)}</div>
<h2 class="sec">Channel quality — YTD</h2>
${chartbox('c_fun_ch',540)}
<div style="margin-top:14px">${tbl([{t:'Channel'},{t:'FTDs',r:1},{t:'PBA %',r:1},{t:'Fraud %',r:1},{t:'Dup %',r:1},{t:'APD2+ %',r:1},{t:'PP %',r:1},{t:'Avg PP score',r:1},{t:'Net PLTV/FTD',r:1}], chRows)}</div>
