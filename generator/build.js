...
${D.atlModel?recCard(8,'brand / ATL','grey','Spend ATL to amplify brand search — and ring-fence a measurement test',
  `ATL (£${gbpM(D.atlModel.atlH1).slice(1)} H1 incl 1.5% agency commission, ${D.atlModel.mediaMix[0].spsh}% TV) isn't trackable per placement and returns only <b>~${D.atlModel.ftd.base.roi.toFixed(2)}x</b> on 12-month PLTV, so justify it on multi-year brand value — not last-click CPA. <b>Actions:</b> keep TV as the core but tilt the response weeks to <b>10s</b> (~23% cheaper per incremental FTD, £${num(D.atlModel.spotLen[0].cpi)} vs £${num(D.atlModel.spotLen[1].cpi)}); <b>uncap brand PPC / Organic / Direct during heavy TV weeks</b> (~62% of the modelled halo lands there — don't let bid caps throttle the demand TV creates); consolidate the small-budget long tail (Sponsorship, Online Video, AVOOH, OOH, LED/partnership boards) into <b>2–3 testable properties</b>; and <b>ring-fence ~10–15% for a regional hold-out or national on/off burst</b> — the only way to turn the ATL model from scenario into measured ROI. Full detail on the <b>ATL model</b> tab.`):''}
...
if(D.funnel){
  ...
  const chR=FN.ch.map(r=>({...r,pbaP:pcf(r.pba,r.f),ppP:pcf(r.pp8,r.f),ppRegP:(r.regs&&r.pp8reg!=null)?pcf(r.pp8reg,r.regs):null,apdP:pcf(r.apd2,r.fm),a0P:r.apd0f?pcf(r.apd0,r.apd0f):null,seonP:(r.regs&&r.seon!=null)?pcf(r.seon,r.regs):null,dupaP:(r.regs&&r.dupa!=null)?pcf(r.dupa,r.regs):null,ufiP:pcf(r.ufi,r.f),dupmP:pcf(r.dupm,r.f)})).sort((a,b)=>b.f-a.f);
  const chRegTot=chR.reduce((a,r)=>({p:a.p+(r.pp8reg||0),g:a.g+((r.pp8reg!=null&&r.regs)||0)}),{p:0,g:0}); const ppRegB=pcf(chRegTot.p,chRegTot.g);
  ...
  const pillHi=(v,g,a)=>`<span class="pill ${v<g?'green':v<a?'amber':'red'}">${pct1(v/100)}</span>`;   // higher = worse
  const pillLo=(v,g,a)=>`<span class="pill ${v>=g?'green':v>=a?'amber':'red'}">${pct1(v/100)}</span>`;  // higher = better
  const moRows=moR.map((r,i)=>({cells:[r.m, num(r.ftds), r.pba!=null?num(r.pba):'—', r.pba!=null?`<span class="pill ${r.pbaP>=14?'red':r.pbaP>=12.5?'amber':'green'}">${r.pbaP.toFixed(1)}%</span>`:'—', num(r.apd2), pillLo(r.apdP,48,44), num(r.pp8), pct1(r.ppP/100), r.qore!=null?num(r.qore):'—', r.qP!=null?pct1(r.qP/100):'—', gbp(div(moTot[i+1].p, r.ftds))]}));
  ...
  const clRows=moR.map((r,i)=>{ const est=r.apd0==null; const a0=est?Math.round(r.ftds*a0Rate):r.apd0; const pv=r.pba||0; const clean=r.ftds-pv-a0; const p=moTot[i+1].p; return {cells:[r.m+(est?' ~':''), num(r.ftds), r.pba!=null?num(r.pba):'—', num(a0), r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pillHi(r.apd1/r.ftds*100,30,35):'—', num(clean), pillLo(clean/r.ftds*100,82,79), gbpK(p), gbp(div(p,r.ftds)), `<b>${gbp(div(p,clean))}</b>`]}; });
  const clTot=moR.reduce((a,r,i)=>{ const a0=r.apd0!=null?r.apd0:Math.round(r.ftds*a0Rate); a.f+=r.ftds; a.pba+=(r.pba||0); a.a0+=a0; a.a1+=(r.apd1||0); a.clean+=r.ftds-(r.pba||0)-a0; a.p+=moTot[i+1].p; return a; },{f:0,pba:0,a0:0,a1:0,clean:0,p:0});
  clRows.push({cls:'tot',cells:['YTD', num(clTot.f), num(clTot.pba), num(clTot.a0), num(clTot.a1), pct1(clTot.a1/clTot.f), num(clTot.clean), pct1(clTot.clean/clTot.f), gbpM(clTot.p), gbp(div(clTot.p,clTot.f)), `<b>${gbp(div(clTot.p,clTot.clean))}</b>`]});
  ...
  const wkRows=wkR.map((r,i)=>({cells:[r.w+(i===wkR.length-1?' (WTD)':''), num(r.ftds), num(r.apd2), pillLo(r.apdP,48,44), r.apd0!=null?num(r.apd0):'—', r.apd0!=null?pillHi(r.apd0/r.ftds*100,6,8):'—', r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pillHi(r.apd1/r.ftds*100,30,35):'—', num(r.pp8), pillLo(r.ppP,ppB,ppB*0.85), wkP[r.w]?gbp(div(wkP[r.w],r.ftds)):'—']}));
  const chRows=chR.map(r=>({cells:[r.ch, num(r.f), num(r.apd2), pillLo(r.apdP,48,44), r.apd0!=null?num(r.apd0):'—', r.apd0!=null?pillHi(r.apd0/r.fm*100,6,8):'—', r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pillHi(r.apd1/r.fm*100,52,56):'—', num(r.pp8), `<span class="pill ${r.ppP>=ppB?'green':r.ppP>=ppB*0.75?'amber':'red'}">${r.ppP.toFixed(1)}%</span>`, r.pp8reg!=null?num(r.pp8reg):'—', r.ppRegP!=null?`<span class="pill ${r.ppRegP>=ppRegB?'green':r.ppRegP>=ppRegB*0.75?'amber':'red'}">${r.ppRegP.toFixed(1)}%</span>`:'—', r.avgpp.toFixed(2), div(r.pn,r.fm)?gbp(div(r.pn,r.fm)):'—']}));
...
${(D.funnel.casc?(()=>{ const C=D.funnel.casc;
  ...
  return `<div class="callout">Reads left→right as a value cascade per channel: <b>Legitimate registrations → PP 8–10 regs → FTDs → PP 8–10 FTDs → Qore FTDs → net revenue (to date)</b>. This is a <b>single registration cohort</b>: legitimate registrations in-window (by registration channel, excluding accounts later closed as fraud/duplicates), then the <i>same players</i> followed through — so every stage is a strict subset and FTDs never exceed registrations. Reg→FTD % = cohort deposit conversion (~62% blended 4-week; the ~52% on total regs was depressed by fraud/duplicate signups). PP 8–10 reg % = high-potential share of legitimate registrations (pre-deposit); Qore % = FTDs reaching £1,000+ paid wagering. Pills are green/amber/red vs the blended rate. Qore &amp; net revenue are realized to-date and still maturing for recent cohorts (see the YTD view below). Channels under 200 regs hidden.</div>
${chartbox('c_fun_casc',300)}
<div style="margin-top:14px">${tbl([{t:'Channel'},{t:'Legit regs',r:1},{t:'PP 8–10 reg',r:1},{t:'% reg',r:1},{t:'FTD',r:1},{t:'Reg→FTD %',r:1},{t:'PP 8–10 FTD',r:1},{t:'% FTD',r:1},{t:'Qore FTD',r:1},{t:'% FTD',r:1},{t:'Net rev (TD)',r:1},{t:'Rev/FTD',r:1}], rows)}</div>
<p class="note">Cascade window: ${D.funnel.cascWindow}. Legitimate registrations &amp; PP from <code>attributed_registrations</code> ⋈ <code>dim_player</code> (last-click at registration; <b>excludes CLOSED/FRAUD, DUPLICATE_* and AML/TAC closures</b> — reconciles ~±2% with the mart's legitimate_registrations); FTD, PP 8–10 FTD, Qore &amp; net revenue are those same cohort players followed to date (Qore = cumulative paid wagers &gt; £1,000; value = realized net revenue / paid NGR). Every stage is a strict subset of registrations, so FTDs ≤ registrations. Modelled PLTV isn't available per-player — see the channel-quality table above for modelled PLTV/FTD. Blended: ${pct1(pp8B/100)} of regs are PP 8–10, ${pct1(ftdB/100)} of regs deposit, ${pct1(qoreB/100)} of FTDs reach Qore, £${r0(ppfB)} net rev/FTD to date.</p>`;
})():'')}
...
<div class="callout">Same cascade over the full year, so <b>Qore &amp; net revenue are mature</b> — compare against the 4-week view above to see how much the recent-cohort numbers will still climb (YTD Qore ${pct1(qoreB/100)} of FTDs vs the 4-week read).</div>
${chartbox('c_fun_casc_ytd',300)}
<div style="margin-top:14px">${tbl([{t:'Channel'},{t:'Legit regs',r:1},{t:'PP 8–10 reg',r:1},{t:'% reg',r:1},{t:'FTD',r:1},{t:'Reg→FTD %',r:1},{t:'PP 8–10 FTD',r:1},{t:'% FTD',r:1},{t:'Qore FTD',r:1},{t:'% FTD',r:1},{t:'Net rev (TD)',r:1},{t:'Rev/FTD',r:1}], rows)}</div>
<p class="note">${D.funnel.cascYtdWindow}. Sources as the 4-week cascade. Blended: ${pct1(pp8B/100)} of regs PP 8–10, ${pct1(ftdB/100)} of regs deposit, ${pct1(qoreB/100)} of FTDs reach Qore, £${r0(ppfB)} net rev/FTD.</p>`;
})():'')}
...
<h2 class="sec">Where to spend ATL — recommendations</h2>
<div class="callout"><b>Read first:</b> ATL isn't trackable per placement, so these are judgement calls anchored to the model's halo estimate and standard media economics — <b>directional, to be confirmed by a test</b> (point 5), not a fitted ROI.</div>
<ol style="margin:8px 0 4px;padding-left:20px;line-height:1.5">
<li style="margin-bottom:8px"><b>Keep TV as the core, but shift the response weeks to 10s.</b> TV is <b>83%</b> of ATL spend and carries most of the halo. 10s spots cost <b>~23% less per incremental FTD (£${num(AM.spotLen[0].cpi)} vs £${num(AM.spotLen[1].cpi)})</b> — airtime scales with duration but a 10s still delivers ~65% of the response. Tilt acquisition-weighted bursts toward 10s; keep 30s for brand-building.</li>
<li style="margin-bottom:8px"><b>Weight ATL to amplify brand search, Direct &amp; Organic — and don't cap those channels while TV is live.</b> ~<b>${AM.breakevenShare>0?'62':'62'}%</b> of the modelled halo lands in PPC Brand, Organic Search and Direct. The cheapest way to waste TV is to let brand-search budgets or bid caps throttle the demand it creates — uncap brand PPC during heavy TV weeks so the lift is captured, not lost.</li>
<li style="margin-bottom:8px"><b>Concentrate the long tail, don't spread it.</b> Sponsorship, Online Video, AVOOH, OOH and the LED/partnership placements (SportFIVE, QPR, Bournemouth, DAZN, Ladbible…) are each 0–3% of spend and individually unmeasurable. Consolidate into <b>2–3 properties you can actually read in a test</b> rather than a thin spread across a dozen.</li>
...
${chartbox('c_atl_med')}
<p class="note"><b>Impact split by medium = spend share × an effectiveness weight</b> (relative halo per £: video/addressable formats — AVOOH, online video — weighted above linear TV; radio, OOH and LED below). <b>TV is ~${AM.mediaMix[0].spsh}% of ATL spend</b>. Spend is now <b>H1 actuals</b>: TV from the agency Day Analysis export (net media cost, £${gbpM(AM.mediaMix[0].sp).slice(1)}), all other media from the H1 MMM media file (20 Jul 2026, daily H1 actuals). <b>All spend includes the 1.5% agency commission on top of the media cost.</b> Effectiveness weights are judgement calls, not measured — a hold-out or on/off test is still needed to size each medium properly. All value at £${AM.ppf} net PLTV/FTD.</p>
<h2 class="sec">ATL impressions &amp; efficiency (H1)</h2>
${(()=>{ const m=AM.mediaMix.slice().sort((a,b)=>b.imp-a.imp); const mx=Math.max(...m.map(r=>r.imp));
  const bar=v=>`<div style="background:var(--line);border-radius:3px;height:9px"><div style="background:var(--blue);height:9px;border-radius:3px;width:${(v/mx*100).toFixed(1)}%"></div></div>`;
  const rows=m.map(r=>({cells:[ r.m, (r.imp/1e6).toFixed(1)+'M', r.impsh.toFixed(1)+'%', bar(r.imp), '£'+r.cpm.toFixed(2) ]}));
  rows.push({cls:'tot',cells:['TOTAL ATL', (AM.atlImpTot/1e9).toFixed(2)+'bn', '100%', '', '£'+AM.atlCpm.toFixed(2)]});
  return tbl([{t:'ATL medium'},{t:'H1 impressions',r:1},{t:'% of impressions',r:1},{t:'Share'},{t:'CPM',r:1}], rows);
})()}
<p class="note"><b>${(AM.atlImpTot/1e9).toFixed(2)}bn ATL impressions in H1 at a blended £${AM.atlCpm.toFixed(2)} CPM.</b> Reach is dominated by <b>TV (${AM.mediaMix[0].impsh.toFixed(1)}% of impressions)</b> and <b>radio</b> — together ~88% of all ATL impressions, with radio by far the cheapest at £${AM.mediaMix.find(x=>x.m==='Radio').cpm.toFixed(2)} CPM. But <b>impression volume ≠ impact</b>: radio's cheap mass reach still sits below TV on the effectiveness weight, so value is driven by the halo model, not raw delivery. <b>Other (LED/perimeter)'s £${Math.round(AM.mediaMix.find(x=>x.m.startsWith('Other')).cpm)} CPM is a data artefact</b> — its impression count is under-reported in the file, not a sign of inefficiency. Impressions are gross delivery (not de-duplicated for reach/frequency); TV from the agency Day Analysis export, all other media from the H1 MMM media file (daily H1 actuals, 20 Jul 2026). CPM = spend incl. 1.5% commission ÷ impressions.</p>
<h3 class="subsec">Sponsorship — modelled conversion impact by placement (H1)</h3>
${(D.atlModel.sponBreak?(()=>{ const b=D.atlModel.sponBreak;
  ...
  rows.push({cls:'tot',cells:['Sponsorship (H1)', gbpK(t.sp), (t.imp/1e6).toFixed(1)+'M', '£'+Math.round(t.sp/t.imp*1000), num(t.incf), gbp(t.sp/t.incf), gbpK(t.incp), roip(t.incp/t.sp)]});
  return tbl([{t:'Placement'},{t:'H1 spend',r:1},{t:'Impressions',r:1},{t:'CPM',r:1},{t:'Modelled incr FTDs',r:1},{t:'Cost / incr FTD',r:1},{t:'Incr net PLTV',r:1},{t:'12m ROI',r:1}], rows);
})():'')}
<p class="note"><b>Pushing the model one level below media type — illustrative only.</b> The Sponsorship medium's modelled incremental FTDs are split across the placements <b>pro-rata by H1 spend</b> (same response weight), so cost per incremental FTD is identical by construction (£${num(AM.sponBreak[0].cpi)}) — the split shows <i>scale</i>, not relative effectiveness. Spend &amp; impressions are full-H1 file actuals: <b>DAZN £${num(AM.sponBreak[0].cpm)} CPM</b> the largest property, then Discovery, LADbible and Sky Sports News. <b>Sky Sports News impressions look under-reported in the file</b> (CPM reads implausibly high) — treat its efficiency as unknown. Like all ATL it's untracked: no FTD is genuinely attributable to a placement, and all sit at <b>~${AM.sponBreak[0].roi.toFixed(2)}x on 12-month PLTV</b> (in line with blended ATL) — brand plays, not direct response. A placement-level on/off test is the only route to a real per-property number.</p>
...
${chartbox('c_atl_grp')}
<p class="note">Equivalent 30-second GRPs from the BARB linear-TV export, now covering <b>all of H1 — ${num(AM.tvGrpH1)} GRPs Jan–Jun</b> (Q1 ${num(AM.tvGrpQ1)} + Q2 ${num(AM.tvGrpQ2)}). TV spend is now the <b>actual net media cost</b> from the agency Day Analysis export, and the BARB delivery reconciles with that export's equivalent TVRs (${num(AM.tvGrpH1)} vs the file's ~1,518). GRPs vary week to week — Q2 ran heavier (peak ~87 GRPs w/c 27 Apr) — but against noisy organic demand still can't pin down a response curve, so the halo remains a scenario, not a fitted coefficient.</p>
<h2 class="sec">TV spot-length effectiveness (H1, actual spend)</h2>
${(()=>{ const s=AM.spotLen;
...
if(D.revshare){
  ...
  // Jul MTD recomputed with gap-filled affiliate spend (same lagging-day gap-fill the rest of the dashboard uses), so it reconciles with the MTD-by-channel tab
  let julRow=null;
  if(R.jul && R.jul.aff_gross!=null){ const j=R.jul, aGf=j.aff_sp+AFF_GAP_28, tGf=j.sp+AFF_GAP_28, n15=j.gross_all-0.15*j.aff_gross, n10=j.gross_all-0.10*j.aff_gross; julRow={m:j.m,sp:tGf,net15:Math.round(n15),net10:Math.round(n10),ltv15:n15/tGf,ltv10:n10/tGf,affltv15:j.aff_gross*0.85/aGf,affltv10:j.aff_gross*0.90/aGf,uplift:Math.round(0.05*j.aff_gross)}; }
  const _rvA=R.rows.concat(julRow?[julRow]:[]); EMBED.revshare={ m:_rvA.map(r=>r.m), ltv15:_rvA.map(r=>+r.ltv15.toFixed(3)), ltv10:_rvA.map(r=>+r.ltv10.toFixed(3)), a15:_rvA.map(r=>+r.affltv15.toFixed(3)), a10:_rvA.map(r=>+r.affltv10.toFixed(3)) };
  ...
  if(julRow) rvr.push({cells:['Jul MTD*', gbpM(julRow.sp), gbpM(julRow.net15), gbpM(julRow.net10), pill(julRow.ltv15), pill(julRow.ltv10), dlt(julRow.ltv15,julRow.ltv10), pill(julRow.affltv15), pill(julRow.affltv10)]});
...
${chartbox('c_rev')}
<p class="note">Reducing the haircut to 10% lifts <b>H1 blended LTV:CAC from ${R.tot.ltv15.toFixed(2)} to ${R.tot.ltv10.toFixed(2)}</b> (+${(R.tot.ltv10-R.tot.ltv15).toFixed(3)}, ~+${((R.tot.ltv10/R.tot.ltv15-1)*100).toFixed(1)}%) and recognises <b>£${(R.tot.uplift/1000).toFixed(0)}k more net PLTV</b> across H1. At the <b>affiliate channel level</b> it rises from ${R.tot.affltv15.toFixed(2)} to ${R.tot.affltv10.toFixed(2)} — <b>still below 1.0 for H1 overall</b>, though Apr &amp; May individually tip past break-even at 10%. So the change flatters affiliate economics but doesn't fix them: the sub-break-even months stay sub-break-even. Pure valuation re-basis of existing PLTV; no change to spend, volume or player quality. <b>Jul MTD (1–15)*</b> is partial and immature (recent-cohort PLTV revises up) so it sits outside the H1 total. Affiliate spend is gap-filled for the lagging feed day (same basis as the MTD-by-channel tab), so the affiliate figures reconcile across tabs. Green ≥1.0 · amber 0.8–1.0 · red &lt;0.8.</p>`;
}

if(D.tvEff){ const TV=D.tvEff, B=TV.bingo, C=TV.casino, M=TV.model;
  const shB=Math.round(B.spend/(B.spend+C.spend)*100), shC=100-shB;
  const totRows=[
    {cells:['Bingo', gbpM(B.spend), num(B.spots), num(B.grp), (B.imp/1e6).toFixed(0)+'M', '£'+B.cpt.toFixed(2), gbp(B.cpg), shB+'%']},
    {cells:['Casino (incl. spon)', gbpM(C.spend), num(C.spots), num(C.grp), (C.imp/1e6).toFixed(0)+'M', '£'+C.cpt.toFixed(2), gbp(C.cpg), shC+'%']},
    {cls:'tot',cells:['Total TV', gbpM(B.spend+C.spend), num(B.spots+C.spots), num(B.grp+C.grp), ((B.imp+C.imp)/1e6).toFixed(0)+'M', '£'+((B.spend+C.spend)/(B.imp+C.imp)*1000).toFixed(2), gbp((B.spend+C.spend)/(B.grp+C.grp)), '100%']} ];
  const mxG=Math.max(...TV.byDow.map(r=>r.bG+r.cG));
  const bar2=(b,c)=>`<div style="display:flex;height:10px;width:100%;border-radius:3px;overflow:hidden;background:var(--line)"><div style="background:var(--blue);width:${(b/mxG*100).toFixed(1)}%"></div><div style="background:var(--pink);width:${(c/mxG*100).toFixed(1)}%"></div></div>`;
  const dowRows=TV.byDow.map(r=>({cells:[ r.dow, r.bG.toFixed(0), r.cG.toFixed(0), bar2(r.bG,r.cG), num(r.ftd), '£'+r.bCpt.toFixed(2), '£'+r.cCpt.toFixed(2) ]}));
  panes.stv = `<h2 class="sec">TV effectiveness — Bingo vs Casino ads <span style="color:var(--muted);font-weight:600">(H1 2026 · day-of-week)</span></h2>
<div class="callout"><b>Bingo ads are the more cost-efficient FTD driver.</b> Bingo buys reach at <b>£${B.cpt.toFixed(2)} CPT / ${gbp(B.cpg)} per GRP</b> versus Casino's <b>£${C.cpt.toFixed(2)} / ${gbp(C.cpg)}</b> — roughly <b>${(C.cpg/B.cpg).toFixed(1)}× cheaper</b> for the same rating point — and carries an equal-or-stronger day-level association with total FTDs. Casino takes ${shC}% of TV spend for ${Math.round(C.grp/(B.grp+C.grp)*100)}% of the GRPs because it runs on premium (Saturday sport-heavy) inventory. That's only worth it if casino-acquired players are materially higher value — which a total-FTD view can't see. <b>Directional read:</b> TV is upper-funnel and its halo isn't cleanly identifiable (see ATL model).</div>
<h3 class="subsec">The buy — H1 delivery by ad type</h3>
${tbl([{t:'Ad type'},{t:'Spend',r:1},{t:'Spots',r:1},{t:'GRPs',r:1},{t:'Impressions',r:1},{t:'CPT',r:1},{t:'£ / GRP',r:1},{t:'% spend',r:1}], totRows)}
<h3 class="subsec">By day of week — pressure vs response <span style="color:var(--muted);font-weight:600">(bars: <span style="color:var(--blue)">bingo</span> + <span style="color:var(--pink)">casino</span> GRPs)</span></h3>
${tbl([{t:'Day'},{t:'Bingo GRPs',r:1},{t:'Casino GRPs',r:1},{t:'GRP mix'},{t:'Total FTDs',r:1},{t:'Bingo CPT',r:1},{t:'Casino CPT',r:1}], dowRows)}
<p class="note"><b>Effectiveness read (correlational).</b> Over ${181} days of H1, adstocked TV pressure (2-wk half-life) correlates with daily FTDs at <b>r=${M.corrB}</b> (bingo) and <b>r=${M.corrC}</b> (casino) — both weak, and the two schedules are <b>${(M.collin*100).toFixed(0)}% collinear</b>, so per-type response can't be cleanly separated. A day-level regression controlling for weekend &amp; payday (<b>R²=${M.r2}</b>) attributes ~<b>${M.olsB.toFixed(1)} FTDs per bingo GRP</b> vs ~<b>${M.olsC.toFixed(1)} per casino GRP</b>; combined with cost that is ~<b>${M.perKB.toFixed(2)} FTDs per £1k</b> on bingo vs ~<b>${M.perKC.toFixed(2)}</b> on casino. The gap is driven mostly by bingo's cheaper inventory, not a proven per-GRP response edge. Bingo runs weekday-heavy; casino peaks Saturday (sport). Not a causal lift measurement — a regional hold-out or on/off burst is the only way to confirm. TV spend incl. 1.5% agency commission; FTDs = total (all channels), source <code>attribution_spend_metrics</code>; GRPs = 30-sec-equivalent national from the agency Day Analysis export.</p>`;
}


if(D.plans){ const PL=D.plans, M=PL.months, gf=PL.gf, bv2=PL.bv2, ac=PL.actual, N=PL.asOfMo;
  const sumTo=(a,n)=>a.slice(0,n).reduce((x,y)=>x+y,0);
...
${(()=>{ const mu=v=>`<span class="pill ${Math.abs(v)<=10?'green':Math.abs(v)<=25?'amber':'red'}">${(v>=0?'+':'')+v.toFixed(0)}%</span>`;
...
<p class="note">Variance = actual ÷ plan − 1 for each month (negative = under plan). Spend variance is RAG-flagged by distance from plan in either direction (green within 10%, amber within 25%, red beyond); FTD variance is RAG-flagged on shortfall (green at/above plan, amber within 10% below, red &gt;10% below). July is excluded here because it is only partial to ${ASOF}. </p>
...
${(()=>{ const W=PL.weekly;
  ...
  const mu=v=>v==null?'—':`<span class="pill ${Math.abs(v)<=10?'green':Math.abs(v)<=25?'amber':'red'}">${(v>=0?'+':'')+v.toFixed(0)}%</span>`;
...
${(()=>{ const B=D.blockDetail;
  ...
  return tbl([{t:'Channel / sub-channel'},{t:'Role'},{t:'Jan',r:1},{t:'Feb',r:1},{t:'Mar',r:1},{t:'Apr',r:1},{t:'May',r:1},{t:'Jun',r:1},{t:'Jul',r:1},{t:'Aug',r:1},{t:'Sep',r:1},{t:'Oct',r:1},{t:'Nov',r:1},{t:'Dec',r:1},{t:'FY',r:1}], rows);
})()}
<p class="note">Full-year FY26 blockplan — every channel (<b>bold</b>) and sub-channel (prefixed ·), full 12 months. Monthly values in \u00a3000s; FY column in \u00a3m. <b>Role</b> = the sheet's Role-of-Media tag (some performance channels are dual Awareness/Conversion; audio/sponsorship sub-lines can differ from their parent). Sub-channels sum into their parent. Source: MrQ FY26 Blockplan Google Sheet.</p>`;
}
...
function mkChart(canvasId,cfg){
  ...
  const ex=(typeof Chart!=='undefined')&&Chart.getChart?Chart.getChart(el):null; if(ex) ex.destroy();
...
function buildPane(id){
  ...
  if(id==='satl' && EMBED.atl){
    ...
    if(EMBED.atl.chAlloc){ const a=EMBED.atl.chAlloc;
      ...
      ]},options:baseOpts({indexAxis:'y',plugins:{title:{display:true,text:'ATL-attributed incremental FTDs & net PLTV by channel (base case) — independent scales'}},scales:{x:{position:'bottom',beginAtZero:true,title:{display:true,text:'Incremental FTDs (blue bars)',color:COL.blue},ticks:{color:COL.blue}},x2:{position:'top',beginAtZero:true,grid:{drawOnChartArea:false},title:{display:true,text:'Incremental net PLTV £ (green bars)',color:COL.green},ticks:{color:COL.green,callback:v=>'£'+Math.round(v/1000)+'k'}}}})});
    }
    ...
    if(EMBED.atl.spotLen){ const s=EMBED.atl.spotLen;
      ...
      ]},options:baseOpts({indexAxis:'y',plugins:{title:{display:true,text:'TV spot length — incremental FTDs vs cost/incr FTD'}},scales:{x:{position:'bottom',beginAtZero:true,title:{display:true,text:'Incremental FTDs (blue bars)',color:COL.blue},ticks:{color:COL.blue}},x2:{position:'top',beginAtZero:true,grid:{drawOnChartArea:false},title:{display:true,text:'Cost / incr FTD £ (pink bars)',color:COL.pink},ticks:{color:COL.pink,callback:v=>'£'+v}}}})});
    }
  }
  if(id==='s8'){
