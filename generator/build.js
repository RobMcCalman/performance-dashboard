...
if(D.funnel){
  ...
  const clRows=moR.map((r,i)=>{ const est=r.apd0==null; const a0=est?Math.round(r.ftds*a0Rate):r.apd0; const pv=r.pba||0; const clean=r.ftds-pv-a0; const p=moTot[i+1].p; return {cells:[r.m+(est?' ~':''), num(r.ftds), r.pba!=null?num(r.pba):'—', num(a0), r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pct1(r.apd1/r.ftds):'—', num(clean), pct1(clean/r.ftds), gbpK(p), gbp(div(p,r.ftds)), `<b>${gbp(div(p,clean))}</b>`]}; });
  ...
  const wkRows=wkR.map((r,i)=>({cells:[r.w+(i===wkR.length-1?' (WTD)':''), num(r.ftds), num(r.apd2), pct1(r.apdP/100), r.apd0!=null?num(r.apd0):'—', r.apd0!=null?pct1(r.apd0/r.ftds):'—', r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pct1(r.apd1/r.ftds):'—', num(r.pp8), pct1(r.ppP/100), wkP[r.w]?gbp(div(wkP[r.w],r.ftds)):'—']}));
