...
if(D.funnel){
  ...
  const fyt=FN.mo.reduce((a,r)=>({ftds:a.ftds+r.ftds,pba:a.pba+r.pba,apd2:a.apd2+r.apd2,pp8:a.pp8+r.pp8,regs:a.regs+r.regs,seon:a.seon+r.seon,dupa:a.dupa+r.dupa,ufi:a.ufi+r.ufi,cfr:a.cfr+r.cfr,dupm:a.dupm+r.dupm,apd0:a.apd0+(r.apd0||0),a0f:a.a0f+(r.apd0!=null?r.ftds:0)}),{ftds:0,pba:0,apd2:0,pp8:0,regs:0,seon:0,dupa:0,ufi:0,cfr:0,dupm:0,apd0:0,a0f:0});
  ...
  // PBA (freeloaders)
  const pFirst=moR[0], pLast=moR[moR.length-1];
  if(pLast.pbaP-pFirst.pbaP>=3) flags.push({s:'amber',t:`<b>Current PBA climbing:</b> ${pFirst.pbaP.toFixed(1)}% of FTDs in ${pFirst.m} → ${pLast.pbaP.toFixed(1)}% in ${pLast.m} (official ThoughtSpot series) — freeloader share rising steadily all year.`});
  ...
  const wkRows=wkR.map((r,i)=>({cells:[r.w+(i===wkR.length-1?' (WTD)':''), num(r.regs), r.seonP.toFixed(1)+'%', r.dupaP.toFixed(1)+'%', num(r.ftds), num(r.ufi), r.ufiP.toFixed(1)+'%', num(r.apd2), pct1(r.apdP/100), num(r.pp8), pct1(r.ppP/100)]}));
  const chRows=chR.map(r=>({cells:[r.ch, r.regs?num(r.regs):'—', r.seonP!=null?`<span class="pill ${r.seonP>2*seonB?'red':r.seonP>1.2*seonB?'amber':'green'}">${r.seonP.toFixed(1)}%</span>`:'—', r.dupaP!=null?`<span class="pill ${r.dupaP>dupaB+5?'red':r.dupaP>dupaB+2?'amber':'green'}">${r.dupaP.toFixed(1)}%</span>`:'—', num(r.f), r.a0P!=null?`<span class="pill ${r.a0P>1.5*a0B?'red':r.a0P>1.2*a0B?'amber':'green'}">${r.a0P.toFixed(1)}%</span>`:'—', `<span class="pill ${r.ufiP>2*ufiB?'red':r.ufiP>1.25*ufiB?'amber':'green'}">${r.ufiP.toFixed(1)}%</span>`, r.dupmP.toFixed(2)+'%', pct1(r.apdP/100), `<span class="pill ${r.ppP>=ppB?'green':r.ppP>=ppB*0.75?'amber':'red'}">${r.ppP.toFixed(1)}%</span>`, r.avgpp.toFixed(2), div(r.pn,r.fm)?gbp(div(r.pn,r.fm)):'—']}));
...
<p class="note">Rules — red: monthly SEON reg-fraud >1.5× YTD, channel SEON >2× blended, UFI ≥2× the prior-8-week rate, channel UFI >2× blended. Amber: Current PBA up ≥3pts vs January, UFI ≥1.5× baseline, duplicate auto-blocks >3pts above YTD (month) or >5pts (channel), manual duplicates >2× blended, channel PP or APD2+ >25% below blended, weekly PP >15% / APD2+ >10% below YTD, Qore down >1.5pts MoM. Thresholds live in build.js.</p>
