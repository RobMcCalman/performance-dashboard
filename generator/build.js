...
if(D.funnel){
  ...
  const clRows=moR.map((r,i)=>{ const est=r.apd0==null; const a0=est?Math.round(r.ftds*a0Rate):r.apd0; const clean=r.ftds-r.pba-a0; const p=moTot[i+1].p; return {cells:[r.m+(est?' ~':''), num(r.ftds), num(r.pba), (est?'~':'')+num(a0), num(clean), pct1(clean/r.ftds), gbpK(p), gbp(div(p,r.ftds)), `<b>${gbp(div(p,clean))}</b>`]}; });
  const clTot=moR.reduce((a,r,i)=>{ const a0=r.apd0!=null?r.apd0:Math.round(r.ftds*a0Rate); a.f+=r.ftds; a.pba+=r.pba; a.a0+=a0; a.clean+=r.ftds-r.pba-a0; a.p+=moTot[i+1].p; return a; },{f:0,pba:0,a0:0,clean:0,p:0});
