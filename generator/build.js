...
${(D.funnel.casc?(()=>{ const C=D.funnel.casc;
  ...
  return `<div class="callout">Reads left→right as a value cascade per channel: <b>Legitimate registrations → PP 8–10 regs → FTDs → PP 8–10 FTDs → Qore FTDs → net PLTV</b>. <b>Registrations exclude accounts later closed as fraud (SEON) or duplicates</b> — so reg→FTD is a genuine conversion rate (blended ~70%, not the ~52% you get on total regs; Direct especially). PP 8–10 reg % = high-potential share of legitimate registrations (pre-deposit). Reg→FTD can exceed 100% for brand/high-intent channels because registrations and FTDs are attributed independently (different last-click point + time window) — read it as a directional conversion strength, not a strict cohort rate; Reg→FTD % = deposit conversion; Qore % = FTDs going on to wager £1,000+ (of FTDs). Pills are green/amber/red vs the blended rate. Qore &amp; PLTV are to-date and still maturing for recent cohorts. Channels under 200 regs hidden.</div>
${chartbox('c_fun_casc',300)}
<div style="margin-top:14px">${tbl([{t:'Channel'},{t:'Legit regs',r:1},{t:'PP 8–10 reg',r:1},{t:'% reg',r:1},{t:'FTD',r:1},{t:'Reg→FTD %',r:1},{t:'PP 8–10 FTD',r:1},{t:'% FTD',r:1},{t:'Qore FTD',r:1},{t:'% FTD',r:1},{t:'Net PLTV',r:1},{t:'PLTV/FTD',r:1}], rows)}</div>
<p class="note">Cascade window: ${D.funnel.cascWindow}. Legitimate registrations &amp; PP from <code>attributed_registrations</code> ⋈ <code>dim_player</code> (last-click at registration; <b>excludes CLOSED/FRAUD, DUPLICATE_* and AML/TAC closures</b> — reconciles ~±2% with the mart's legitimate_registrations); FTD &amp; Qore from <code>attributed_ftds</code> ⋈ gameplay (Qore = cumulative paid wagers &gt; £1,000); net PLTV net of the 15% affiliate revshare. Blended: ${pct1(pp8B/100)} of regs are PP 8–10, ${pct1(ftdB/100)} of regs deposit, ${pct1(qoreB/100)} of FTDs reach Qore, £${r0(ppfB)}/FTD.</p>`;
})():'')}
