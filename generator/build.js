...
if(D.funnel){
  ...
  const moRows=moR.map(r=>({cells:[r.m, num(r.ftds), num(r.pba), `<span class="pill ${r.pbaP>=2?'red':r.pbaP>=1?'amber':'green'}">${r.pbaP.toFixed(2)}%</span>`, r.a0P!=null?num(r.apd0):'—', r.a0P!=null?r.a0P.toFixed(1)+'%':'—', num(r.ufi), `<span class="pill ${r.ufiP>=2*ufiB?'red':r.ufiP>=1.25*ufiB?'amber':'green'}">${r.ufiP.toFixed(1)}%</span>`, num(r.dupm), r.dupmP.toFixed(2)+'%', num(r.apd2), pct1(r.apdP/100), num(r.pp8), pct1(r.ppP/100), r.qore!=null?num(r.qore):'—', r.qP!=null?pct1(r.qP/100):'—']}));
  moRows.push({cls:'tot',cells:['YTD', num(fyt.ftds), num(fyt.pba), pct1(pbaB/100), num(fyt.apd0)+' (Jan–May)', pct1(a0B/100), num(fyt.ufi), pct1(ufiB/100), num(fyt.dupm), pct1(dupmB/100), num(fyt.apd2), pct1(apdB/100), num(fyt.pp8), pct1(ppB/100), num(qTot)+' (Jan–Jun)', pct1(qB/100)]});
  ...
  const chRows=chR.map(r=>({cells:[r.ch, r.regs?num(r.regs):'—', r.seonP!=null?`<span class="pill ${r.seonP>2*seonB?'red':r.seonP>1.2*seonB?'amber':'green'}">${r.seonP.toFixed(1)}%</span>`:'—', r.dupaP!=null?`<span class="pill ${r.dupaP>dupaB+5?'red':r.dupaP>dupaB+2?'amber':'green'}">${r.dupaP.toFixed(1)}%</span>`:'—', num(r.f), `<span class="pill ${r.pbaP>Math.max(1.5,3*pbaB)?'red':r.pbaP>2*pbaB?'amber':'green'}">${r.pbaP.toFixed(2)}%</span>`, r.a0P!=null?`<span class="pill ${r.a0P>1.5*a0B?'red':r.a0P>1.2*a0B?'amber':'green'}">${r.a0P.toFixed(1)}%</span>`:'—', `<span class="pill ${r.ufiP>2*ufiB?'red':r.ufiP>1.25*ufiB?'amber':'green'}">${r.ufiP.toFixed(1)}%</span>`, r.dupmP.toFixed(2)+'%', pct1(r.apdP/100), `<span class="pill ${r.ppP>=ppB?'green':r.ppP>=ppB*0.75?'amber':'red'}">${r.ppP.toFixed(1)}%</span>`, r.avgpp.toFixed(2), div(r.pn,r.fm)?gbp(div(r.pn,r.fm)):'—']}));
...
${chartbox('c_fun_mo')}
<div style="margin-top:14px">${tbl([{t:'Month'},{t:'FTDs',r:1},{t:'PBA',r:1},{t:'PBA %',r:1},{t:'APD0',r:1},{t:'APD0 %',r:1},{t:'UFI',r:1},{t:'UFI %',r:1},{t:'Dup manual',r:1},{t:'Dup man %',r:1},{t:'APD2+',r:1},{t:'APD2+ %',r:1},{t:'PP 8–10',r:1},{t:'PP %',r:1},{t:'Qore FTDs',r:1},{t:'Qore %',r:1}], moRows)}</div>
