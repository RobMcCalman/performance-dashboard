...
if(D.funnel){
  ...
  const chRows=chR.map(r=>({cells:[r.ch, num(r.f), num(r.apd2), pillLo(r.apdP,48,44), r.apd0!=null?num(r.apd0):'—', r.apd0!=null?pillHi(r.apd0/r.f*100,6,8):'—', r.apd1!=null?num(r.apd1):'—', r.apd1!=null?pillHi(r.apd1/r.f*100,30,35):'—', num(r.pp8), `<span class="pill ${r.ppP>=ppB?'green':r.ppP>=ppB*0.75?'amber':'red'}">${r.ppP.toFixed(1)}%</span>`, r.avgpp.toFixed(2), div(r.pn,r.fm)?gbp(div(r.pn,r.fm)):'—']}));
...
${(()=>{ const D2=D.funnel.drag; const vp=v=>`<span class="pill ${v<0.8?'red':v<1.0?'amber':'green'}">${v.toFixed(2)}</span>`;
  ...
  const aff=D2.aff.map(r=>({cells:[ r.n, gbpK(r.s), num(r.f), vp(r.ltv), gbp(r.ppf), pct1(r.apd1p/100), pct1(r.svp/100), verdict(r.ltv) ]}));
  const adg=D2.adg.map(r=>({cells:[ r.label, gbpK(r.s), num(r.f), vp(r.ltv), pct1(r.apd2p/100), pct1(r.svp/100), verdict(r.ltv) ]}));
  return `<h3 class="subsec">Affiliate partners</h3>${tbl([{t:'Affiliate'},{t:'Spend',r:1},{t:'FTDs',r:1},{t:'Net LTV:CAC',r:1},{t:'PLTV/FTD',r:1},{t:'One-day %',r:1},{t:'Savvy %',r:1},{t:'Verdict'}], aff)}
<h3 class="subsec">Other campaigns / ad groups (paid, ex-affiliate)</h3>${tbl([{t:'Channel · ad group'},{t:'Spend',r:1},{t:'FTDs',r:1},{t:'Net LTV:CAC',r:1},{t:'APD2+ %',r:1},{t:'Savvy %',r:1},{t:'Verdict'}], adg)}`;
})()}
<p class="note">The spend most dragging blended quality/value down, last 4 weeks, net of the 15% affiliate revshare. <b>Affiliates:</b> the premium big-three (nicasinoprem, newriseproject, moonshot, ~£964k/4wk) sit below breakeven with the highest freeloader (savvy) rates; the Simona Todoroska / Moar volume books (digadvfree, digadvsocial, bca22hmcuk) clear breakeven only on rock-bottom CPA but leak 55–60% one-day players. <b>Campaigns:</b> Meta App installs/purchase and Meta web-volume/incremental, UAC iOS slots, and PPC brand-other convert well below break-even last-click (time-decay re-credits some app value — verify before hard-cutting). <b>Verdict</b>: Cut/renegotiate &lt;0.8 net · Watch 0.8–1.0 · Keep ≥1.0. One-day % (affiliates) is maturation-inflated for recent cohorts; ranking is reliable.</p>
